...
class InputParser {
  ...
  private:
    ...
    template <typename T> T read_required_option(string root_name, string option_name, string extra_msg = "") const {
    ...
    template <typename T> T read_optional_option(string root_name, string option_name, T default_value) const {
        if (root_node[root_name][option_name]) {
            return root_node[root_name][option_name].as<T>();
        } else {
    ...
    EnumType read_required_enum_option(string root_name, string option_name,
                                       const std::map<string, EnumType> &enum_map) const {
        if (root_node[root_name][option_name]) {
